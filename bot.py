...
async def create(ctx):
    if not is_admin(ctx):
        return await ctx.send(embed=Embed(color=discord.Color.red(), description="❌ You don't have access to this command."))
...
async def access_slot(ctx):
    if not isinstance(ctx.channel, discord.DMChannel):
        return await ctx.send(embed=Embed(color=discord.Color.red(), description="❌ Use this command in DMs."))
